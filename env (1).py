...
# MODIFIE — ajout des coefficients tactiles
...
def _leap_reset_state_fn(key, data_batch, reset_mask, spec, use_dr, base_mjx, grasp_cache, grasp_cache_size):
    ...
    if use_grasp_cache:
        ...
        cached_states = grasp_cache[cache_indices]
...
class MJXLeapHandEnv:
    def __init__(self, xml_path: str, num_envs: int, key: jax.Array,
        ...
        # NOUVEAU — attributs tactiles initialises a zero
        # Ils seront mis a jour par env_wrapper.step() avant chaque appel a self.step()
        ...
        # MODIFIE — ajout des 5 coefficients tactiles dans partial
    ...
    def _sample_push(self, key: jax.Array, num_envs: int) -> jnp.ndarray:
    ...
    def _sample_duration(self, key: jax.Array, num_envs: int, lo: int, hi: int) -> jnp.ndarray:
        return jr.randint(key, (num_envs,), lo, hi + 1)

    def _advance_wrench(self, key: jax.Array, reset_mask: jnp.ndarray):
    ...
    def step(self, actions: jnp.ndarray):
        ...
        # NOUVEAU — extra_state_fn injecte les features tactiles dans le state dict
        # pour que _leap_reward_fn puisse y acceder via state['contact_fingers'] etc.
        ...
        (
            ...
            extra_state_fn=_extra_state,    # NOUVEAU
        )
        ...
        action_rate = -self.action_rate_scale * jnp.sum((smoothed_actions - prev_for_rate) ** 2, axis=-1)
        ...
        self.prev_actions = jnp.where(reset_mask[:, None], jnp.zeros_like(smoothed_actions), smoothed_actions)
    ...
    def reset(self, env_ids: jnp.ndarray = None):
        ...
        onset = self._sample_duration(wrench_key, self.num_envs, self.wrench_rest_steps[0], self.wrench_rest_steps[1])
